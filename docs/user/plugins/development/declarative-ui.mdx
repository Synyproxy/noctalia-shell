...
| Constructor | Props |
|-------------|-------|
| `ui.column` / `ui.row` | `gap`, `padding`, `paddingH`, `paddingV`, `align` (`start`/`center`/`end`/`stretch`), `justify` (`start`/`center`/`end`/`space_between`), `fill`, `radius`, `border`, `borderWidth`, `minWidth`, `minHeight`, `onClick`, `onHover`, `tooltip` |
...
| `ui.image` | `path` (plugin-relative, `~`, or absolute), `width`, `height`, `radius`, `fit` (`contain`/`cover`/`stretch`), `border`, `borderWidth`, `onClick`, `onHover`, `tooltip` |
| `ui.box` | `fill`, `radius`, `border`, `borderWidth`, `softness`, `width`, `height`, `onClick`, `onHover`, `tooltip` |
...
- **Tooltips**: `ui.button` accepts a `tooltip` string, shown on hover. `ui.box`, `ui.row`, `ui.column`, and `ui.image`
  also accept `tooltip` (requires <PluginApiBadge feature="container-tooltip" />), which wraps the node in a hover
  target so a plain container or glyph can carry one. Dropping the prop on a later render clears it. Tooltips appear in
  bar widgets and panels; desktop widgets do not show them.
...
injects four standard settings for every `[[panel]]` entry. You do **not** declare them as `[[panel.setting]]` unless
you need a custom key. They appear in **Settings → Plugins** (gear on the plugin row) with the same controls as built-in
panels: a segmented **Attached / Floating** toggle, **Position** and **Layer** dropdowns (only when floating), and
**Open Near Click**.
...
| `effect` | _(none, manifest only)_ | Post-process the panel is composited through: `none` (default) or `crt` (row static, band tears, scanlines, vignette). Tune it from the script with `panel.setEffect{ intensity = 1, speed = 1, static_rate = 0.012, ... }`; keys follow `CrtEffectParams` in the shell source and unknown keys are ignored |
...
| `layer` | `<entry>_layer` | `top` (default) or `overlay`; only applies when floating (optional manifest default requires <PluginApiBadge feature="panel-layer" />) |

Declare defaults on the `[[panel]]` table; users override through the plugin settings GUI or
`[plugin_settings."author/plugin"]`. The host supplies every standard setting even when the manifest declares no
default, so plugin maintainers do not need to add anything for users to choose a layer. Position and open-near-click
follow the same rules as built-in panels (pinned screen positions disable open-near-click).

**Layer** controls the floating surface's layer-shell layer. `top` keeps the panel under fullscreen windows; `overlay`
keeps it above them, which is what an on-screen keyboard or heads-up panel needs. An attached panel ignores this
setting and sits at its host bar's configured layer.
