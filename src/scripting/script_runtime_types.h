...
namespace scripting {
  ...
  // Host-only metadata attached to the callback generated by a pointer event.
  // Luau never receives these values directly; a panel API may consume them to
  // create native transient UI tied to the originating input serial.
  struct ScriptPointerContext {
    float x = 0.0F;
    float y = 0.0F;
  ...
  enum class ScriptContextMenuItemKind : std::uint8_t {
    Action,
    Separator,
    Header,
  };
  ...
  struct ScriptContextMenuRequest {
    std::vector<ScriptContextMenuItem> items;
  ...
  struct ScriptResult {
    ...
    // One-shot host request. Unlike ScriptPatch this is never merged into the
    // runtime replay state and therefore cannot reopen after a late subscribe.
